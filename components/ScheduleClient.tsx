...
export default function ScheduleClient({ initialGames }: { initialGames: any[] }) {
  // 1. Calculate the active week (first week that has unplayed games)
  const firstUnplayedGame = initialGames.find(g => !g.vScore || g.vScore === '');
  const activeWeek = firstUnplayedGame ? firstUnplayedGame.week.toString() : initialGames[0]?.week.toString();

  // 2. Initialize state with the active week
  const [selectedWeek, setSelectedWeek] = useState(activeWeek);

  const weeks = Array.from(new Set(initialGames.map(g => g.week.toString())))
    .sort((a, b) => Number(a) - Number(b));

  const filteredGames = initialGames.filter(g => g.week.toString() === selectedWeek);

  return (
    <div className="space-y-8">
      {/* WEEK NAVIGATION */}
      <div className="flex gap-2 overflow-x-auto pb-4 no-scrollbar sticky top-16 z-40 bg-[#f8fafc]/80 backdrop-blur-md py-4">
        {weeks.map(week => (
          <button
            key={week}
          ...
          >
            Week {week}
            {week === activeWeek && (
               <span className="ml-2 inline-block w-1.5 h-1.5 bg-blue-500 rounded-full animate-pulse"></span>
            )}
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filteredGames.length > 0 ? (
          ...
          <div className="col-span-full py-20 text-center">
            <p className="text-slate-400 font-black uppercase italic tracking-widest text-sm">No games found for this week.</p>
          </div>
        )}
      </div>
    </div>
