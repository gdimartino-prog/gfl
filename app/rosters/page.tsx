          ...
          {['OFF', 'DEF'].map((unit) => (
            <div key={unit} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden h-fit">
              <div className={`px-4 py-3 font-bold text-white flex justify-between items-center ${unit === 'OFF' ? 'bg-blue-800' : 'bg-red-800'}`}>
                ...
                <span className="bg-white/20 px-2 py-0.5 rounded text-xs">{groups[unit]?.length || 0}</span>
              </div>
              <div className="p-2">
                  ...
                  <tbody>
                    {(groups[unit] || []).map((p: any, i: number) => (
                      <tr key={i} className="border-b last:border-0 border-gray-50 hover:bg-gray-50">
                        ...
                        <td className="py-2 px-3 text-sm text-gray-800 uppercase font-semibold">{p.name}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="p-2">
                {groups.SPEC.map((p: any, i: number) => (
                  <div key={i} className="flex px-3 py-2 border-b last:border-0 border-gray-50 text-sm">
            ...
            <div className="bg-amber-50 rounded-xl shadow-sm border border-amber-200 overflow-hidden">
              <div className="px-4 py-3 font-bold text-amber-900 bg-amber-200 flex justify-between items-center">
                <span className="tracking-tight">DRAFT CAPITAL</span>
                <span className="bg-amber-900/10 px-2 py-0.5 rounded text-xs font-black">{data.picks.length}</span>
              </div>
              <div className="p-4 space-y-3">
                {data.picks.length > 0 ? (
                  data.picks.map((pick: any, i: number) => {
                    const isViaTrade = pick.originalTeam?.toUpperCase() !== selectedTeam.toUpperCase();
                    return (
                      <div key={i} className="bg-white border border-amber-100 p-3 rounded-lg shadow-sm flex justify-between items-center">
                        <div>
                          <p className="text-[10px] text-amber-600 font-black uppercase tracking-tighter">
                  ...
                  <p className="text-center text-xs text-amber-600 italic">No picks currently held.</p>
                )}
              </div>
            </div>
          </div>
