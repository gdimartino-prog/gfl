...
export default function TradeBuilder() {
 ...
 const handleTrade = async () => {
  ...
  try {
    ...
    const formatDraftPicks = (draftPickList: string[], allDraftPicks: DraftPick[]) =>
      draftPickList
        .map((dp) => {
          const pick = allDraftPicks.find(
            (p) => `${p.year}-${p.round}` === dp
          );
          if (!pick) return dp;
          return `Draft Pick Year: ${pick.year}, Round: ${pick.round}`;
        })
        .join(', ');
    ...
    if (fromPlayers.length || fromDraftPicks.length) {
      const fromEntry = [
        ...
        formatDraftPicks(fromDraftPicks, draftPicks)
      ]
        .filter(Boolean)
        .join(', ');
    ...
    if (toPlayers.length || toDraftPicks.length) {
      const toEntry = [
        ...
        formatDraftPicks(toDraftPicks, draftPicks)
      ]
        .filter(Boolean)
        .join(', ');
