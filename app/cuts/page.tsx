...
export default function CutsPage() {
  ...
  const stats = useMemo(() => {
    const getStatsFor = (type: string) => {
      ...
      list.forEach(p => { posMap[p.position] = (posMap[p.position] || 0) + 1; });
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50">
      ...
      <p className="text-sm font-black uppercase tracking-widest text-slate-400 animate-pulse">Initializing...</p>
    </div>
  );

  return (
    <div className="max-w-6xl mx-auto p-4 md:p-8 space-y-6 bg-gray-50 min-h-screen text-slate-900 font-sans">
      ...
      {selectedTeam && (
        <>
          {/* 2. DASHBOARD STATS (NEW TOP PLACEMENT) */}
                  ...
                  return (
                    <tr key={pKey} className="hover:bg-gray-50/50">
                      <td className="px-6 py-4">
                        <a href={`https://www.google.com/search?q=${encodeURIComponent(`${p.first} ${p.last} fantasy`)}`} target="_blank" rel="noreferrer" className="font-bold uppercase text-slate-800 hover:text-blue-600">{p.first} {p.last}</a>
...
function StatCard({ title, data, accent }: any) {
  return (
      ...
      <div className="flex flex-wrap gap-1 mt-1">
        {Object.entries(data.posMap).map(([pos, count]) => (
          <span key={pos} className="text-[9px] font-bold bg-gray-50 px-1.5 py-0.5 rounded border border-gray-100">
            {pos}:<span className="text-blue-500 ml-0.5">{count}</span>
          </span>
        ))}
      </div>
    </div>
...
function StatusButton({ label, active, type, onClick }: any) {
  ...
  return (
    <button onClick={onClick} className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase border-2 transition-all ${active ? activeClasses[type] : 'bg-white text-gray-300 border-gray-100 hover:border-gray-300 hover:text-gray-500'}`}>
      {label}
    </button>
  );
}

function Counter({ label, current, max, color }: any) {
  return (
